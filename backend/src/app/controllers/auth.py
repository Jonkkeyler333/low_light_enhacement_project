from fastapi import APIRouter, Depends, HTTPException, Response, Depends
...
@router.post('/logout', summary = "Logout a user", description = "Logout the currently authenticated user by clearing the JWT token cookie.")
async def logout_user(response: Response):
    response.delete_cookie(key = "access_token", path = "/")

@router.post('/me', response_model = UserResponse, summary = "Get current user", description = "Retrieve the currently authenticated user's information.")
